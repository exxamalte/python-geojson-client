...
class TestNswRuralFireServiceFeed(unittest.TestCase):
    ...
    def test_update_ok(self, mock_session, mock_request):
        ...
        assert (
            repr(feed) == "<NswRuralFireServiceFeed("
            "home=(-31.0, 151.0), "
    ...
    def test_update_ok_with_categories(self, mock_session, mock_request):
        ...
        feed = NswRuralFireServiceFeed(
            home_coordinates, filter_categories=["Category 1"]
        )
        status, entries = feed.update()
        assert status == UPDATE_OK
        self.assertIsNotNone(entries)
    ...
    def test_feed_manager(self, mock_session, mock_request):
        ...
        feed_manager = NswRuralFireServiceFeedManager(
            _generate_entity, _update_entity, _remove_entity, home_coordinates, None
        )
        assert (
            repr(feed_manager) == "<NswRuralFireServiceFeedManager("
            "feed=<NswRuralFireServiceFeed("
            "home=(-31.0, 151.0), "
            "url=https://www.rfs.nsw.gov.au/feeds/"
            "majorIncidents.json, radius=None, "
        ...
        feed_manager.update_override({"categories": ["Category 2"]})
        entries = feed_manager.feed_entries
        self.assertIsNotNone(entries)
        assert len(entries) == 1
